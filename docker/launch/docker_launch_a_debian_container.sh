...
docker run ppc64le/debian ps aux

# Show the containers again
docker ps -a

# Create another container and change the image
docker run --name=mycontainer2 ppc64le/debian mkdir /tmp/foo

# Show the changes made by the container
docker diff mycontainer2
...
docker run -n mycontainer3 -i -t debian_v2 bash 
